...
//	}
	...
	public SingularValueDecomposition(Matrix Arg) {
		...
		int nct = Math.min(m - 1, n);
		int nrt = Math.max(0, Math.min(n - 2, m));
		int numberOfThreads = ConfigurationParameters.numberOfThreads;
		int ksPerThread = Math.max(nct, nrt) / numberOfThreads;
		...
		/*******************************************************/
		/*******************************************************/
		IntStream.range(0, numberOfThreads).parallel().forEach(th -> {
			for (int k = ksPerThread * th; k < ksPerThread * (th + 1); k++) {
				if (k < nct) {
		...
		for (int k = ksPerThread * numberOfThreads; k < Math.max(nct, nrt); k++) {
			if (k < nct) {
